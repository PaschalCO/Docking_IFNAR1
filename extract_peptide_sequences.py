...
# Ruta donde están almacenadas las carpetas de péptidos
...
for folder in os.listdir(base_dir):
    ...
    if os.path.exists(peptide_pdb) and os.path.getsize(peptide_pdb) > 0:  # Verifica si el archivo no está vacío
        try:
            structure = parser.get_structure(folder, peptide_pdb)
            sequence = ""
            residues_found = 0  # Contador de residuos válidos
                ...
                for chain in model:
                    ...
                    if chain.id == chain_id:  # Asegurar que no estamos tomando residuos de varias cadenas
                        for residue in chain:
                            if PDB.is_aa(residue, standard=True):  # Solo considera aminoácidos estándar
                                try:
                                    sequence += PDB.Polypeptide.three_to_one(residue.resname)
                                    residues_found += 1  # Contar residuos válidos
                                except KeyError:
                                    print(f"Residuo desconocido en {folder}: {residue.resname}")

            if residues_found > 0:
                seq_dict[folder] = sequence  # Solo guardar si hay residuos válidos
            else:
